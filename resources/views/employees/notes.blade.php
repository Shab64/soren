                    ...
                    <div class="box">
                        ...
                        <div class="box-body notesHeight">
                            ...
                            <div class="row mt-3">
                                ...
                                <div class="col-md-3">
                                    <div class="notesBody">
                                        @if($note['type'] === 'Employee' && isset(Auth::guard('web')->user()->id))
                                        @if(Auth::guard('web')->user()->id === $note['owner_id'])
                                        <div class='delNotes'>
                                            <a href="javascript:void(0)" onclick="delete_note('Employee','<?= $note['owner_id'] ?>','<?= $note['id'] ?>')" class="btn btn-sm btn-danger"><i class='fa fa-remove'></i></a>
                                        </div>
                                        @endif
                                        @endif
                                            <p>{{ strip_tags($note['note']) }}</p>
                                            <div class="endpara">
                                                @if($note['type'] === 'Employee')
                                                <p>{{time_Ago($time_ago)}}<span> by {{Auth::guard('web')->user()->name}}</span></p>
                                                    @else
            ...
            <div class="modal fade" id="addNote" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <form method="post" action="{{ Route('employee.insertNote') }}">
                            @csrf
...
<script>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form method="POST" action="{{ Route('employee.deleteNote') }}">
                @csrf
