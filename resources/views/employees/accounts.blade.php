                                    ...
                                    @if(!empty($converted_leads))
                                        @foreach($converted_leads as $k=>$converted)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{url('employee/view/account-view/'.$converted['id'])}}" class="btn btn-primary btn-sm" title="view details"><i class="fa fa-eye"></i></a>
                                                    <a href="javascript:void(0)" onclick="delete_('<?= url('employee/lead/destroy/' . $converted['id']) ?>')" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></a>
                                                </td>
                                            </tr>
                                        @endforeach
                                    @endif
...
<script>
    ...
    const  accountStatus=(type,leadID)=>{
        ...
        let status = '';
        if(check ===true){status = 'yes';
        }else {status = 'no'}
        let _token = '{{ csrf_token() }}';
        $.post("{{url('employee/changeAccountStatus')}}", {_token, leadID, type, status}, function(e) {
            console.log('success');
        });
    }
</script>
